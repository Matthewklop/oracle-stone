/* ============================================================================
 * stone2lua.c — Stone → Lua Transpiler
 *
 * Converts Stone source to Lua. No compile step needed.
 * Usage: ./stone2lua < input.st > output.lua
 *        ./stone2lua file.st > file.lua
 *
 * Properties:
 *   - Same Stone grammar (stack-based, comma-free, paren-free)
 *   - Outputs valid Lua 5.1+
 *   - print → io.write(), printn → print()
 *   - loop → for i=0,N-1 do, if/else → if/else with then
 *   - fn → function, var → local assignment
 * ============================================================================
 */
...
/* Parse a function call: name arg arg ... → name(arg, arg, ...) */
/* Caller has already consumed and emitted the function name. */
...
static void parse_stmt(Parser *p) {
    ...
    switch (t.type) {
        ...
        case TOK_VAR: {
            ...
            emit_indent_only(e); emit_str(e, "local "); emit_str(e, name.text);
            lex_peek(l, &t);
            if (t.type == TOK_IDENT || t.type == TOK_NUMBER || t.type == TOK_STRING) {
                emit_str(e, " = ");
                /* Use parse_expr to read value and optional operator chain */
                parse_expr(p, 0);
                /* After parse_expr, check if there are more args (function call pattern) */
                lex_peek(l, &t);
                if (t.type == TOK_IDENT || t.type == TOK_NUMBER || t.type == TOK_STRING) {
                    parse_call(p);
                }
        ...
        case TOK_PRINT: case TOK_PRINTN: {
            ...
            emit_indent_only(e);
            if (nl) {
                emit_str(e, "print("); parse_expr(p, 0); emit_str(e, ")\n");
            } else {
                emit_str(e, "io.write("); parse_expr(p, 1); emit_str(e, ")\n");
            }
            break;
        }
        ...
        case TOK_IF: {
            lex_next(l, &t);
            emit_indent_only(e); emit_str(e, "if "); parse_cond(p); emit_str(e, " then\n");
            e->indent++;
            ...
            lex_skip_newlines(l); lex_expect(l, TOK_END, &t);
            emit_line(e, "end"); break;
        }

        case TOK_LOOP: {
            lex_next(l, &t); Token v; lex_expect(l, TOK_IDENT, &v);
            emit_indent_only(e); emit_str(e, "for "); emit_str(e, v.text); emit_str(e, " = 0, ");
            parse_expr(p, 0); emit_str(e, "-1 do\n");
            e->indent++;
            while (1) { lex_skip_newlines(l); lex_peek(l, &t);
                if (t.type == TOK_END || t.type == TOK_EOF) break; parse_stmt(p); }
            e->indent--;
...
int main(int argc, char **argv) {
    ...
    /* Header */
    emit_line(&e, "-- Generated by stone2lua");
    emit_line(&e, "");
    ...
    /* If a main function exists, call it at bottom */
